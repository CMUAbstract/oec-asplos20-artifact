# Usage: python3 plot_f6c.py /path/to/src/ /path/to/dst/
#
...
if len(sys.argv)==3:
  ...
  print("Usage: python3 plot_f6c.py /path/to/src/ /path/to/dst/")
  exit()

# Equatorial
eq_xvalues = []
eq_yvalues = []
with open(src+'sim-cs/logs/cs-planet-eq.csv','r') as infile:
...
ns_xvalues = []
ns_yvalues = []
with open(src+'sim-cs/logs/cs-planet-ns.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
un_xvalues = []
un_yvalues = []
with open(src+'sim-cs/logs/cs-planet-un.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
fig = plt.figure()
ax  = plt.axes(\
 title='Close-spaced, 97.3 inclination, 215 nanosatellites', \
...
ax.legend()
fig.savefig(dst+'downlink-deficit-cs.pdf',bbox_inches='tight')
plt.close(fig)
