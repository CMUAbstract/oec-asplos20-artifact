# Usage: python3 plot_f8b.py /path/to/src/ /path/to/dst/
#
...
if len(sys.argv)==3:
  ...
  print("Usage: python3 plot_f8b.py /path/to/src/ /path/to/dst/")
  exit()

# Baseline
base_xvalues = []
base_yvalues = []
with open(src+'csfp-base/logs/csfp-coverage.csv','r') as infile:
...
mcap_xvalues = []
mcap_yvalues = []
with open(src+'csfp-mcap/logs/csfp-coverage.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
mpow_xvalues = []
mpow_yvalues = []
with open(src+'csfp-mpow/logs/csfp-coverage.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
mprc_xvalues = []
mprc_yvalues = []
with open(src+'csfp-mprc/logs/csfp-coverage.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
ax.legend()
fig.savefig(dst+'csfp-coverage.pdf',bbox_inches='tight')
plt.close(fig)
