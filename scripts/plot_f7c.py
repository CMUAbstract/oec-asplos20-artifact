# Usage: python3 plot_f7c.py /path/to/src/ /path/to/dst/
#
...
if len(sys.argv)==3:
  ...
  print("Usage: python3 plot_f7c.py /path/to/src/ /path/to/dst/")
  exit()

# Baseline
base_xvalues = []
base_yvalues = []
with open(src+'fstp-base/logs/fstp-latency.csv','r') as infile:
...
mcap_xvalues = []
mcap_yvalues = []
with open(src+'fstp-mcap/logs/fstp-latency.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
mpow_xvalues = []
mpow_yvalues = []
with open(src+'fstp-mpow/logs/fstp-latency.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
mprc_xvalues = []
mprc_yvalues = []
with open(src+'fstp-mprc/logs/fstp-latency.csv','r') as infile:
  lines = infile.readlines()
  lines = lines[1:]
  for line in lines:
    line_split = line.split(',')
...
ax.legend()
fig.savefig(dst+'fstp-latency.pdf',bbox_inches='tight')
plt.close(fig)
