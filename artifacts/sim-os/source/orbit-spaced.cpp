...
// Satellite configuration files should produce an orbit-spaced formation.
...
int main(int argc, char** argv) {
  ...
  while(centisecondCount<1080000) { // three hours, i.e. approximately 2 revs
    ...
    //// For orbit-spaced, each satellite captures every N data frame
    ...
    if(distance>=satId2Threshold[LEAD_SAT_ID]) {
      frameCount++;
      bool triggerTime = (frameCount%satellites.size()==0 ? true : false);
      if(triggerTime) {
        frameCount = 0;
        log.evnt(
         comsim::LogLevel::INFO,currDateTime.toString(),"trigger-time"
        );
      } else { // a frame has advanced, register with lead sat w/o getting data
        satId2Sensor[LEAD_SAT_ID]->setPrevSensePosn(currPosn);
        satId2Sensor[LEAD_SAT_ID]->setPrevSenseDateTime(currDateTime);
      }
      for(size_t i=0; i<satellites.size(); i++) {
        if(triggerTime) {
