// fstp-mprc.cpp
// Frame-spaced, tile-parallel more processing
//
...
int main(int argc, char** argv) {
  ...
  // For FSTP, satellites are separated in radial position
  ...
  while(simulate) {
    ...
    for(size_t i=0; i<ehsatellites.size(); i++) {
      ...
      if(ehsPosn<PI) {
        ...
        if(ciPtr->isIdle() && !ciPtr->hasImage() &&
           ...
           jobPtr->getWorkerTaskCount(ciPtr->getWorkerId())==0) {
          // For FSTP, a subset of tasks are claimed
          jobPtr->claimTasks(
           ciPtr->getWorkerId(),
           std::ceil(
    ...
    if(ehsatellites.size()<pipelineDepth &&
      ...
      jetsonTX4.setWorkerId(ehsatellites.size());
      jetsonTX4.setSimTime(simTime_sec);
      jetsonTX4.logEvent(
       "jetson-"+std::to_string(jetsonTX4.getWorkerId())+"-idle-start",
       simTime_sec
      );
      ehsystem.addEnergyConsumer(jetsonTX4);
      // Energy consumer: Chameleon imager
