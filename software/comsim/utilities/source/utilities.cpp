...
#include <utilities.hpp>   // constants
...
namespace comsim { namespace util {
  ...
  double calcSystemNoiseTemp(
   const ChannelType& channelType, const double& centerFrequencyHz
    ...
    // See comsim/documentation/wertz1999space.pdf pg. 558 Table 13-10
    double t_ant = 290.0;
    double t_line = cnst::SMAD_T0*(1.0-cnst::SMAD_LR)/cnst::SMAD_LR;
    double noiseFactor = std::pow(10.0,5.0/10.0);
    if(channelType==ChannelType::DOWNLINK) {
    ...
    } else if(channelType==ChannelType::CROSSLINK) {
      t_ant = 20.0;
      noiseFactor = std::pow(10.0,5.0/10.0);
    } else if(channelType==ChannelType::UPLINK) {
